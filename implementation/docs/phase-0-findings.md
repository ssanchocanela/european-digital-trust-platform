...
| **B7** | No issuance to an **unmodified** official wallet build is possible | The pinned release resolves issuer trust per `VerificationContext` and has neither a trust list nor a classification for a non-qualified EAA, under an `ENFORCE` default; the seven dev anchors are EUDIW-operated with no self-service enrolment for an EAA issuer | Milestone 2 can be built and tested against the engine, but a wallet issuance test needs a **modified** wallet | **Identified at the Milestone 2 gate.** Smallest modification is one line — `forVct("<our vct>", TrustPolicy.Action.INFORM)` — leaving the PID path enforcing; the faithful alternative is to publish an `eaaProviders` LoTE and declare `eaas`. Either way the result is a modified wallet and §8 labelling applies. See [`milestone-2-issuer-trust.md`](milestone-2-issuer-trust.md) |
...
| ~~**Q7**~~ | ~~Does an intermediary need a separate access certificate **per intermediated RP**?~~ | — | **RESOLVED at the Milestone 1 review, and the prompt was right.** ARF 3.0.0 §6.6.5 point 1, citing the **amended** CIR 2025/848 Annex IV point 3(k): an intermediary's access certificate contains "an association to the wallet-relying party that is relying upon the intermediary", and ARF draws the consequence itself — "an intermediary receives a separate set of access certificates for each of its intermediated Relying Parties", with a worked example of 100 Instances × 1000 customers = 100,000 certificates. `RPI_06` alone does not say it; §6.6.5 does. Note the profile lag in the next row: **ETSI TS 119 475 V1.1.1 (2025-10)** does *not* carry this association in the WRPAC — read directly, its Annex B mapping marks the intermediary row as absent from the WRPAC and its Annex E.1 reads Annex IV 3(k) as a certificate-policy reference. Still out of V0 scope |
| **Q7a** | Is the ARF/CIR intermediary data model actually profiled by ETSI TS 119 475? | Decides whether a Wallet can perform the §6.6.5 cross-check from certificates alone | **OPEN, newly raised.** In V1.1.1 (2025-10) `usesIntermediary` *is* profiled (Annex B.2.1 → CIR Annex I.14; Table 10 → `act.sub.id`/`act.sub.name`; `GEN-5.2.4-09`), but the access-certificate side of Annex IV 3(k) is not, and the phrase "Service identifier" does not occur in the document at all — so the Service-identifier half of the §6.6.5 comparison is unprofiled. Likely version lag: ETSI's normative reference is CIR 2025/848 *of 6 May 2025* (unamended) and V1.1.1 is its first publication. Resolve by reading the amending act and any ETSI version later than V1.1.1. Recorded on PR #4 |
...
| ~~**Q10**~~ | ~~How does the pinned Reference Wallet release decide to trust an **issuer** of a non-qualified EAA?~~ | — | **RESOLVED at the Milestone 2 gate, and it is a blocker (B7).** Issuer trust is ETSI LoTE-based and resolved **per `VerificationContext`** (`PID` / `PubEAA` / `QEAA` / `EAA(useCase)`). For a non-qualified EAA the shipped build has no list (`SupportedLists.eaaProviders` exists but the app never populates it), no classification (`classifications(...)` declares only `pids`, in both flavours), and `policy { default(TrustPolicy.Action.ENFORCE) }` — so `evaluateIssuerTrust` throws `IssuerNotTrustedException`. `requireSignedMetadata()` is a fourth, independent gate. The mechanism is **absent by design**, since ARF does not have non-qualified EAA providers publishing to a notified trust list. Full record, including the smallest wallet modification and the *Check Registration Certificates* coupling, in [`milestone-2-issuer-trust.md`](milestone-2-issuer-trust.md) |
