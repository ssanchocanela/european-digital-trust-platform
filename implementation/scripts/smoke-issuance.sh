...
# ## Why this is not two days
#
# It was, and on 16 September 2026 that cost an afternoon. The chain was provisioned on the 13th,
# the certificate expired on the 15th, and everything went on looking healthy: the credential offer
# minted, resolved over public HTTPS, the token endpoint issued a DPoP-bound access token. The
# refusal arrived only at the very last call —
#
#     400 credential_request_denied
#     Certificate validation failed: Certificate expired on 2026-09-15T08:19:35.000Z
#
# — and `GET …/provider-authentication` reported nothing about it, because that report covers trust
# gate (a) and this is the attestation-signing key. So the console showed an issuer blocked only by
# B7 while it had in fact been unable to sign anything for a day.
#
# A short life is not a safety property here: this key is generated by a smoke test, held by a
# development engine, and never leaves it. What a short life buys is a stack that rots quietly
# between sessions. `CERT_DAYS` overrides it for anyone who wants the expiry behaviour on purpose.
...
openssl req -new -x509 -key "$CERT_DIR/key.pem" -out "$CERT_DIR/cert.pem" -days "$CERT_DAYS" \
  -subj "/CN=Smoke Test Issuer/O=Development only/C=NL" 2>/dev/null
chmod 600 "$CERT_DIR"/*.pem
echo "Attestation-signing certificate valid until $(openssl x509 -in "$CERT_DIR/cert.pem" -noout -enddate | cut -d= -f2)."
echo "After that, issuance fails at the LAST call of the flow and nowhere earlier."
