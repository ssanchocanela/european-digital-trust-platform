...
| K3a | **Issuer trust depends on lists loaded by hand.** Until 17 September 2026 presentations were verified with **no** issuer trust check at all (`interop-findings.md` A30); they now fail closed unless the policy names a list this deployment loaded. Loading is `scripts/load-issuer-trust-list.mjs`, run per engine tenant and again at every rollover, and the signer pin in it is **trust on first use** — taken from the published list and checked for consistency, not obtained out of band. Nothing refreshes a loaded list, so a list past its `NextUpdate` keeps being used by the engine until reloaded | Platform-owned list retrieval on a schedule, signer anchors obtained from the Member State notification rather than from the list, and alarms on `NextUpdate` — which is K3 |
...
| K4a | **A platform-operated development PID Provider CA** (`scripts/make-dev-pid-ca.sh`) signs the test PID issuer's attestations, and a TEST list carrying it plus the seven notified dev PID anchors is published on `gh-pages` and loaded into the verifier's engine tenant, so *Identify with PID* v5 accepts those attestations. No Member State notified it (`EW-PIO-01-024`, `OIA_12`); only a wallet built with WD-4 trusts it. Its key lives in `~/.edtp/dev-pid-ca`, unencrypted, on a developer machine. `TEST` only, never in a `PRODUCTION` trust configuration | Nothing: a real PID Provider is notified by a Member State and is not something this platform operates |
| K4b | **The `operator-form` authentic source issues whatever the caller types** — the declared exception to "a subject reference, never attribute values". Guarded by `assertSuppliedAttributesAllowed`: only a `FIXTURE` connector, only under a `TEST` Attestation Provider, refused for every other source. Anyone holding the tenant key can issue a PID-shaped attestation of any content under the development CA | Removal before any `PRODUCTION` use; a real authentic source |
...
| P5 | **Wallet-initiated issuance depends on the test gateway for its form.** The engine's authorization endpoint mints a code for any browser, so the hosted form is mandatory only because the gateway puts it in front and admits a browser only with the platform's HMAC pass (`test-session-gateway.md` §1e). Exposed without the gateway, the form is skipped — and nothing is issued, since the platform holds no values for that session. The engine's `nbf` check has zero tolerance, absorbed by a 3 s hold at PAR and token in demo mode (A29) | An engine-side authorization hook, or an authorization server of our own |
| P6 | **A refused hosted-form or operator-form submission leaves its transaction in `ELIGIBILITY_CHECK`** until the expiry job settles it as `EXPIRED`. No attestation results, but the state reads as in progress | Settle as refused at the point of refusal |
| P7 | **The demonstration bank is a public page that can start presentations.** `apps/demo-bank` holds `HOSTED_VERIFIER_SECRET`, which the platform accepts only for the presentation policies in `HOSTED_VERIFIER_POLICIES`; with it, anyone who reaches the page can make the platform create `SAME_DEVICE` presentations of those policies and read their outcome, including the verified claims, for as long as the presentation lives. It holds no tenant key and cannot choose where the wallet returns — the platform builds that from `HOSTED_VERIFIER_PUBLIC_URL` and looks it up by presentation id. It keeps nothing. | Up only during a test session; synthetic credentials only; its own secret, which the platform refuses to share with the hosted form's; the negative checks probe its host. A production relying party would call the platform with its own tenant credentials from its own back end |
| P8 | **The demonstration environment is exposed permanently** (ADR 0010). The engine's protection rests on the gateway allow-list and the negative checks, with no session time limit. The hosted form, the bank, the shop and the portal can be used by anyone, within per-client limits in each process: the gateway allows 300 requests a minute in general and 30 a minute each for issuance and for presentation responses; the form allows 20 POSTs a minute, the bank 10. The limits are counted per client address from Cloudflare's `CF-Connecting-IP`, trusted because the VM has no inbound path but the tunnel. They are in memory, so a restart forgets them. One coarse Cloudflare edge rule sits in front. While a client profile is on, the client's name and emblem are publicly readable in issuer metadata (ADR 0010 §2) | Synthetic data only; `TEST` only; no inbound ports; nightly reset and scheduled negative checks (ADR 0010 §3; the checks are not yet on a schedule). A production deployment would need distributed rate limiting, abuse monitoring and a real registration certificate |
| P9 | **The client-profile switch on the operator page does not verify the organisation's permission** (ADR 0010 §2). Cloudflare Access verifies *who* is switching: the `Solo operador` policy, with a one-time PIN by e-mail. But there are no roles: anyone added to that policy can switch profiles. The "written permission" box is a declaration, recorded with the requester's identity in the VM journal; nothing checks that the permission exists. The portal also trusts the Access identity header rather than validating the signed `Cf-Access-Jwt-Assertion`, which is safe only because the VM has no inbound path but the tunnel | Accepted for now (decided 24 September 2026). Two hardening options are recorded: (1) make a client profile available only when its configuration carries a reference to the written permission and an expiry date, refused otherwise; (2) validate the Access JWT against the team's public keys |
...
| V4 | **Wallet-initiated issuance holds the typed values in platform memory** between the hosted form and the engine's attribute-provider call — once, for at most the transaction lifetime (10 minutes for the PID policy), never persisted or logged. A restart loses them and the person starts again; a second platform instance would not see them. The engine, as on every issuance, keeps the issued values for its session window (V1) | Acceptable for V0's single process; a shared encrypted store with a short TTL if the platform scales out |
