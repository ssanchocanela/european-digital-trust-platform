...
| **Q10** | How does the pinned Reference Wallet release decide to trust an **issuer** of a non-qualified EAA? | Gates the whole of Milestone 2. The issuance trust path is **not** the WRPAC LoTE that blocker B1 is about, and nothing in Phase 0 verified it — assuming it resolves like the verification path would be the "terminology similarity does not prove conformance" error | **OPEN, and it is the first task of Milestone 2**, before any issuance code. Read the source of `Wallet/Demo_Version=2026.09.42-Demo_Build=42` and Wallet Core `v0.30.2`: which mechanism resolves issuer trust (dev `PubEAAProviders` LoTE, a separate issuer trust store, OpenID4VCI issuer metadata, or nothing), whether a non-qualified EAA differs from a PID, whether a development issuer can be trusted without modifying the build, and what *Check Registration Certificates* changes given `RPRC_22a`/`RPRC_23` and blocker B3. **Then STOP and report** — if it is a blocker, say so rather than designing around it. Decided at the Milestone 1 review; recorded in `CLAUDE.md` §1 |
