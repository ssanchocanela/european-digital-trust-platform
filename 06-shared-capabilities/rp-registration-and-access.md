...
- `[OPEN]` **Possible gap between the ARF/CIR requirements and the ETSI certificate profile.** `[SPECIFICATION]` ARF 3.0.0 §6.6.5 makes intermediary status visible in *both* certificates: (a) per the **amended** CIR 2025/848 Annex IV point 3(k), an intermediary's access certificate contains "an association to the wallet-relying party that is relying upon the intermediary" — from which the ARF draws the consequence that an intermediary needs a separate set of access certificates per intermediated Relying Party; and (b) the intermediated Relying Party's registration certificates show that it uses the intermediary, so the Wallet Unit cross-checks the identifier **and Service identifier** in the registration certificate's `usesIntermediary` field against the access certificate, informs the User on mismatch and may stop the transaction. `[IMPLEMENTATION-EVIDENCE]` **ETSI TS 119 475 V1.1.1 (2025-10)** — the version referenced by ARF 3.0.0, checked directly — profiles only part of that. Its `usesIntermediary` attribute exists (Annex B.2.1, mapped to CIR Annex I.14; Table 10 maps it to `act.sub.id` and `act.sub.name`, "as specified by the intermediary WRPAC"), so half the cross-check is supported. But its Annex B mapping table marks the intermediary-association row as **not present in the WRPAC** and only *optional* in the WRPRC, its Annex E.1 list of WRPAC contents reads Annex IV point 3(k) as "machine processable reference to the applicable certificate policy" rather than an intermediary association, and the document contains **no Relying Party Service identifier at all** — which the ARF's own registration discussion topic lists as a change still to be made to TS 119 475. `[INFERENCE]` The most likely explanation is version lag, not contradiction: the ETSI document's normative reference is CIR 2025/848 *of 6 May 2025* and its change history records V1.1.1 as the first publication (September 2025), so it profiles the pre-amendment text — the same sub-point reference carrying different content in each document. `[OPEN]` Confirm against the amending act itself (not checked here) and against any ETSI version later than V1.1.1 before relying on certificate-level intermediary data. Until then, treat the Service-identifier half of the §6.6.5 cross-check as unprofiled, and do not design a control that assumes an access certificate carries the intermediated Relying Party's identity.
